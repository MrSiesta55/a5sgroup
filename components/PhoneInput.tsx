...
export const COUNTRIES: Country[] = [
  ...
  { code: 'DO', name: 'Dominican Republic', dial: '+1829' },
...
export default function PhoneInput({ value, onChange, placeholder = '70 123 45 67', required }: PhoneInputProps) {
  const defaultCountry = COUNTRIES.find(c => c.code === 'SE')!;
  ...
  // The full value sent to parent: dial code + space + number
  const handleNumberChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    onChange(`${selected.dial} ${e.target.value}`);
  };

  // Extract just the number part for the input display
    ...
    <div className="flex gap-2">
      ...
      <div ref={dropdownRef} className="relative flex-shrink-0">
        <button
          type="button"
          onClick={() => setOpen(v => !v)}
          className="h-full flex items-center gap-1.5 bg-white/5 border border-white/10 rounded-xl px-3 py-3.5 text-white focus:outline-none focus:border-blue-500/60 transition-all duration-200 hover:bg-white/8 min-w-[90px]"
        >
          <span className="text-xl leading-none">{flag(selected.code)}</span>
          <span className="text-sm font-medium text-white/80">{selected.dial}</span>
          <svg
            className={`w-3.5 h-3.5 text-white/40 transition-transform duration-200 ml-0.5 ${open ? 'rotate-180' : ''}`}
        ...
        {/* Dropdown */}
        {open && (
          <div className="absolute top-full left-0 mt-2 w-72 bg-[#0f1117] border border-white/15 rounded-2xl shadow-2xl z-50 overflow-hidden">
            {/* Search bar */}
            ...
            <ul className="max-h-56 overflow-y-auto py-1 scrollbar-thin">
              {filtered.length === 0 ? (
                <li className="px-4 py-3 text-sm text-white/40 text-center">No results</li>
                ...
                <li key={country.code}>
                  <button
                    ...
                    className={`w-full flex items-center gap-3 px-4 py-2.5 text-sm hover:bg-white/8 transition-colors text-left ${
                      selected.code === country.code ? 'bg-white/10 text-white' : 'text-white/80'
                    }`}
                  >
                    <span className="text-lg leading-none w-6 text-center flex-shrink-0">{flag(country.code)}</span>
                    <span className="flex-1 truncate">{country.name}</span>
                    <span className="text-white/40 text-xs font-mono flex-shrink-0">{country.dial}</span>
                  </button>
                </li>
              ))}
            </ul>
      ...
      <input
        ...
        className="flex-1 bg-white/5 border border-white/10 rounded-xl px-4 py-3.5 text-white placeholder-gray-600 focus:outline-none focus:border-blue-500/60 transition-all duration-200"
      />
    </div>
  );
}
